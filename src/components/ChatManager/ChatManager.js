...
const ChatManager = (props) => {
  ...
  const getMessagesHistory = (senderUserName,recieverUserName)=>{
    axios.get(`https://chat-project-server.azurewebsites.net/api/messages/get-messages?senderUserName=${senderUserName}&recieverUserName=${recieverUserName}`,{
      headers:{
  ...
  const joinRoom = async () => {
    ...
    try{
      ...
      .withUrl(`https://chat-project-server.azurewebsites.net/chat`,{accessTokenFactory: ()=> localStorage.getItem('key')})
